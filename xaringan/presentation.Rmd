...
- Rasterwerte können vom Typ `numeric`, `integer`, `logical` oder `factor` sein
  + nicht baer vom typ `character` (um character-werte zu benutzen vorher in `factor` konvertieren)
 ...
 **Beispiel aus der Dokumentation des raster-paket** <sub>1</sub>
...
.footnote[
[1][Hijmans, 2019](https://cran.r-project.org/web/packages/raster/raster.pdf)
]
---
...
- ## Zellenwerte eines `RasterLayers` extrahieren
  ...
  + `bsp_raster[1,1]` gibt eine eindeutige ID für die Zelle der ersten Zeile und Spalte zrück
